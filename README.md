...
Plush carry 沿用两张动态折叠桌和开阔搬运路线，将水桶替换为约 74 cm 高的牛油果
软玩偶。主体使用 MuJoCo 三维柔性体：115 个表面节点负责碰撞和局部压缩，形状约束
使其在释放后回弹，并避免软体节点长期漂移，适合实时遥操作。

reset 会随机化玩偶 0.9–2.0 kg 的重量、软硬度、表面摩擦和初始姿态。柔性体与
胸部、上臂和前臂的接触会进入同一套 episode 数据和 PICO 手柄反馈。装饰性的脸、
果核、手脚和叶片不参与碰撞，避免细小部件造成不稳定接触；场景不自动判定成功。
